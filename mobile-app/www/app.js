...
const API_BASE = 'https://bookmark-manager.exe.xyz:8000';
    ...
    async function loadBookmarks(source = '') {
        ...
        if (source === 'things-to-try') {
            url = API_BASE + '/api/bookmarks?tag=try';
        } else if (source) {
            url = `${API_BASE}/api/bookmarks?source=${source}`;
        } else {
    ...
    async function analyzeBookmark(id) {
        ...
        try {
            const res = await fetch(`${API_BASE}/api/bookmarks/${id}/analyze`, { method: 'POST' });
            const data = await res.json();
    ...
    async function addToTry(id) {
        ...
        try {
            await fetch(`${API_BASE}/api/bookmarks/${id}/tag/try`, { method: 'POST' });
    ...
    async function fetchMeta() {
        ...
        const res = await fetch(API_BASE + '/api/fetch-metadata', {
            method: 'POST',
    ...
    async function loadCollectionBookmarks(collectionId) {
        ...
        const res = await fetch(`${API_BASE}/api/collections/${collectionId}/bookmarks`);
        const bookmarks = await res.json();
    ...
    async function doWebSearch() {
        ...
        const res = await fetch(`${API_BASE}/api/web-search?q=${encodeURIComponent(query)}`);
        const data = await res.json();
    ...
    function loadGitHubConfig() {
        fetch(API_BASE + '/api/github/config').then(r => r.json()).then(data => {
    ...
    async function gitPull() {
        ...
        try {
            const res = await fetch(API_BASE + '/api/github/pull', { method: 'POST' });
            const data = await res.json();
    ...
    async function importInstagramData() {
        ...
        try {
            const res = await fetch(API_BASE + '/api/instagram/import', {
                method: 'POST',
                body: formData
            });
    ...
    async function quickAddURL() {
        ...
        try {
            const metaRes = await fetch(API_BASE + '/api/fetch-metadata', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ url })
            });
    ...
    async function moveSelectedToCategory() {
        ...
        await fetch(API_BASE + '/api/bookmarks/bulk-update', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({
    ...
    async function addSelectedToCollection() {
        ...
        await fetch(`${API_BASE}/api/collections/${collectionId}/bookmarks`, {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({
                bookmark_ids: Array.from(selectedBookmarks)
